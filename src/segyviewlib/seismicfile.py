...
class SeismicFile:
    ...
    def open(filename, file_type=None):
        ...
        if file_type == Filetype.SEGY:
            ...
            if pyzgy is None:
                raise ImportError("File type requires pyzgy. Install optional dependency with pip.")
            handle = pyzgy.open(filename)
            handle.structured = True
            handle.sorting = segyio.TraceSortingFormat.INLINE_SORTING
        elif file_type == Filetype.VDS:
            if pyvds is None:
                raise ImportError("File type requires pyvds. Install optional dependency with pip.")
            handle = pyvds.open(filename)
            handle.structured = True
            handle.sorting = segyio.TraceSortingFormat.INLINE_SORTING
        elif file_type == Filetype.SGZ:
            if seismic_zfp is None:
                raise ImportError("File type requires seismic-zfp. Install optional dependency with pip.")
